...
import PremiumAlert, { AlertType } from './PremiumAlert';
...
import { Check, Loader2, AlertCircle, Sparkles, ShieldX, LogOut, Bell } from 'lucide-react';
import { auth } from '../firebaseConfig';
// Removed modular auth imports to fix type errors
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    // Use compat style auth.onAuthStateChanged
    const unsubscribe = auth.onAuthStateChanged(async (firebaseUser) => {
      setIsLoadingAuth(true);
      if (firebaseUser) {
        // Force refresh the token to get the latest emailVerified status
        // This handles cases where user clicks link in email then comes back to this tab
        try {
        ...
        // SYNC: If Auth says verified but DB says unverified, update DB immediately
  ...
  const addToCart = (product: Product) => {
    ...
    if (product.stock <= 0 || currentQtyInCart >= product.stock) {
      showNotification(`Exquisite rarity: Only ${product.stock} units are currently available for allocation.`, 'error');
      return;
    }
  ...
  const updateCartQuantity = (productId: string, delta: number) => {
    setCart((prev) => {
      ...
      if (delta > 0 && newQty > item.stock) {
        showNotification(`Maximum allocation reached for this unique creation.`, 'error');
        return prev;
      }
  ...
  const renderPage = () => {
    // 1. GLOBAL BAN INTERCEPT - LOCKS ALL ACCESS
            ...
            <div className="max-w-xl bg-white/5 p-16 rounded-[4rem] border border-white/10 backdrop-blur-xl shadow-2xl space-y-12">
                ...
                <div className="space-y-6">
                    ...
                    <p className="text-gray-400 font-light text-lg leading-relaxed italic">
                        ...
                        Your administrative association with the Vroica registry has been suspended. Please contact our system architect for reconciliation.
                    </p>
                </div>
        ...
        return (
            <CheckoutPage 
                ...
                onPlaceOrder={async (orderId) => {
                    setCart([]);
                    await refreshProducts(); 
                    showNotification(`Acquisition recorded officially.`);
                }}
                navigate={navigate}
            />
        );
  ...
  return (
    <div className={`min-h-screen font-sans flex flex-col ${isAdminView || isBannedView ? 'bg-[#080808]' : 'bg-vroica-cream'}`}>
